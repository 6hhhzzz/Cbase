...
**Cbase = 企业知识库中间件**，外部 AI Agent 是一等公民。
...
Cbase 通过 `space_type` 实现双轨隔离，在不影响现有功能的前提下支持 AI 原生的文档治理：
...
```bash
cd ai-service && uv run pytest tests/ -q     # Python: 248 tests (含治理管道 15 + submit_document 8)
cd backend && mvn test                        # Java: 71 单测
cd backend && mvn verify                      # Java: 71 单测 + 11 IT (需要 Docker, Testcontainers 起 PG+Redis)
cd frontend && npm run build && npm run test  # Frontend: 22 tests + coverage
```

### 覆盖率门槛（CI 强制）

所有门槛硬编码在 CI 中，低于阈值构建直接失败：

| 端 | 门槛 | 配置文件 |
|----|------|---------|
| Python | `--cov-fail-under=44` (当前 ~45%) | [ci.yml](.github/workflows/ci.yml#L44) |
| Java | JaCoCo INSTRUCTION ≥ 20% (verify 阶段) | [pom.xml](backend/pom.xml) `jacoco-maven-plugin` check goal |
| Frontend | vitest thresholds: lines≥5%, branches≥20%, functions≥3% (当前 8.4%/38.7%/5.55%) | [vitest.config.js](frontend/vitest.config.js) |

**规则**：功能更新后若覆盖率下滑过门槛，CI 变红 → 必须补测试或上调门槛。门槛可以上调，不能下调（除非同时声明降级原因）。

### IT 集成测试基建注意

- **单例容器模式**：[AbstractIntegrationTest](backend/src/test/java/com/kes/AbstractIntegrationTest.java) 用 `static{}` 启停 PG + Redis 容器，全 JVM 共享。不用 `@Testcontainers/@Container`（会按类启停 + Spring 上下文缓存导致后续类 DataSource 指向死容器 → HikariPool 超时）。
- **Mock I/O 边界**：基类用 `@MockBean MinioClient` + `@MockBean RabbitTemplate`，避免 `@PostConstruct` 连接外部服务。
- **Jackson SNAKE_CASE**：`application.yml` 全局配置 `property-naming-strategy: SNAKE_CASE`，所有 record/DTO 的 JSON 字段名会自动转 snake_case（`accessToken`→`access_token`，`spaceId`→`space_id`）。写测试断言时必须用 snake_case 字段名。
- **Refresh token `jti`**：`JwtUtil.generateRefreshToken()` 使用 `Jwts.builder().id(UUID)` 保证每次 token 唯一，避免同秒轮转碰撞。
- **Redis 缓存 + IT**：`PermissionQueryService.resolveAccessibleKbIds()` 先读缓存。测试中直接 `aceRepo.save()` 绕过了 `AceService` 的缓存失效，需手动 `permissionCache.evict()` 模拟生产行为。
...
# 确认远程可达
git remote -v
# 确保用 SSH（HTTPS 在国内可能被墙）
# 正确: git@github.com:6hhhzzz/Cbase.git
...
9. **文档治理管道**：DocUnderstand（LLM 理解内容 → topics/summary/type）→ DocClassify（Jaccard + LLM → 归入已有 KB 或创建新 KB）→ UpdateKBSummary（聚合 topics → 写 KB 元数据）。外部 Agent 只需按规范提交文档，内部 Cbase 自动处理分类和 KB 管理。
10. **MCP Resource 极简设计**：2 个 Resource（catalog + docs），Agent 不需要术语表、时间范围、单文档元数据细节 — 只需知道有哪些 KB 和每个 KB 有什么文档，然后用 `search_chunks` 精准检索。
11. **CI 覆盖率门禁**：三端均设硬覆盖率门槛（Python 44% / Java 20% / 前端 lines 5%），低于阈值 CI 直接挂。功能更新后覆盖率下滑必须补测试或上调门槛，防止"代码变了测试没跟上"的退化循环。
